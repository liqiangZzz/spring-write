...
/**
 * @ClassName DefaultBeanFactory3
 * @Description 默认的Bean工厂 别名增强
 * @Author liqiang
 * @Date 2025/9/18 16:16
 */
@Slf4j
public class DefaultBeanFactory4 implements BeanFactory, BeanDefinitionRegistry, Closeable {
    ...
    private Object createInstanceByConstructor(BeanDefinition beanDefinition) throws InstantiationException, IllegalAccessException {
        try {
            // 使用新的推荐方式，替代已弃用的 newInstance() 方法
            ...
            log.error("找不到无参构造函数, beanDefinition: " + beanDefinition, e);
            throw new InstantiationException("No default constructor found for bean: " + beanDefinition.getBeanClass().getName());
        }
    }
